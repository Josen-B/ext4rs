...
impl InodeMode {
    pub const DEFAULT_FILE: Self = Self::IFREG.union(Self::IRUSR).union(Self::IWUSR).union(Self::IRGRP).union(Self::IROTH);
    pub const DEFAULT_DIR: Self = Self::IFDIR.union(Self::IRUSR).union(Self::IWUSR).union(Self::IXUSR).union(Self::IRGRP).union(Self::IXGRP).union(Self::IROTH).union(Self::IXOTH);
}
...
impl Inode {
    ...
    pub fn inode_type(&self) -> InodeType {
        ...
        if mode_type == InodeMode::IFREG {
            InodeType::File
        } else if mode_type == InodeMode::IFDIR {
            InodeType::Directory
    ...
    pub fn permissions(&self) -> u16 {
        (self.mode & (InodeMode::IRUSR | InodeMode::IWUSR | InodeMode::IXUSR |
                      InodeMode::IRGRP | InodeMode::IWGRP | InodeMode::IXGRP |
                      InodeMode::IROTH | InodeMode::IWOTH | InodeMode::IXOTH)).bits()
    }
    ...
    pub fn get_block_number(&self, offset: u64, block_size: u32) -> Ext4Result<u32> {
        ...
        if block_index < 12 {
            ...
            Ok(self.block[12])
        } else if block_index < 12 + (block_size as u64 / 4) + ((block_size as u64 / 4) * (block_size as u64 / 4)) {
